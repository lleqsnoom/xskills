...
/**
 * Create docs/staging/plans/ directory and return the full plan file path.
...
function main() {
  ...
  const dir = path.resolve("docs/staging/plans");
  const filename = `${date}-${args.topic}.md`;
  const fullPath = path.join(dir, filename);

  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(fullPath, `# Plan — ${args.topic}\n\n**Date:** ${date}\n**Branch:** ${branch}\n\n---\n\n`);
