...
/**
 * Create .x-skills/apis/ directory and return the full design file path.
 * Auto-detects branch name from git if not provided. Generates timestamp.
 * Logs each step to stderr for verification.
 *
 * Usage:
 *   node save-design.js --topic <slug> [--branch <name>] [--date <YYYY-MM-DDTHHMM>]
 *
...
function log(stage) {
  ...
  process.stderr.write(`[${ts}] [x-api-draft] ${stage}\n`);
}
...
// from a global install ( ~/.agents/skills/x-api-draft/scripts/ ) or a local one
// (.agents/skills/<project>/x-api-draft/scripts/). This lets us find sibling
...
function main() {
  ...
  if (!args.topic) {
    console.error("Usage: node save-design.js --topic <slug> [--branch <name>] [--date <YYYY-MM-DDTHHMM>]");
    process.exit(1);
  }
  ...
  const header = `# API Design — ${args.topic}\n\n**Date:** ${date}\n**Branch:** ${branch}\n**Scope:** \n**Status:** Draft — awaiting user approval\n\n---\n\n`;
  log(`writing design file: ${fullPath} (${header.length} bytes)`);
  fs.writeFileSync(fullPath, header);

  // Verify the write succeeded
  const stats = fs.statSync(fullPath);
  if (stats.size === 0) {
    console.error("ERROR: design file was written but is empty.");
    process.exit(1);
  }

  log(`design ready: ${fullPath}`);
