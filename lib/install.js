 ...
 * An existing skill is left alone unless `force` is set, which replaces it.
 */
async function install(skillName, { force = false } = {}) {
  ...
  if (await dirExists(targetPath)) {
    if (!force) {
      console.log(`Skill "${skillName}" is already installed. Re-run with --force to update it.`);
      return;
    }
    // Replace rather than merge: a skill that dropped a file must not keep it.
 ...
 * An existing skill is left alone unless `force` is set, which replaces it.
 */
async function globalInstall(skillName, { force = false } = {}) {
  const sourceSkill = ensureSkillExists(skillName);
  ...
  if (await dirExists(targetPath)) {
    if (!force) {
      console.log(`Skill "${skillName}" is already installed globally. Re-run with --force to update it.`);
      return;
    }
    await fsp.rm(targetPath, { recursive: true, force: true });
  }
...
/**
 * Get a simple list of skill directory names (for install-all).
 */
